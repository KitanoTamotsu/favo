...
　　URLをAppend to Fileオブジェクトでテキストファイルに追加した後に、OSXの通知を使ってメッセージを表示させます
...
　　こうすることで、ファイルに登録されたURLが全てRandomユーティリティのリストにって寸法さ

　　でもなぜか、workflowの環境変数にセットしたfilenameのホームディレクトリ"~"が使えませんでした。
　　"Users/..."と書けば環境変数でも使えましたが、その場合配布した先の環境ではうまくうごきません。
　　仕方なくスクリプトの中でホームディレクトリをリテラルで記載しました
